-- ════════════════════════════════════════════════════════════════════════
-- Make hidden mean "not counted", everywhere on the dashboard.
--
-- hidden_at was added so cleared drafts left the board and the started_30d
-- tile together. Six metrics never got the memo: they read job_invitations,
-- contractor_quotes and client_quotes directly rather than through the `js`
-- CTE, so a hidden job still moved invites-per-job, prices-per-job, the
-- decline rate, the median time-to-first-price, and the count of contractors
-- invited and pricing.
--
-- Found clearing seven test jobs: started_30d dropped 50 → 44 as intended,
-- while six invitations and three quotes from those same jobs carried on
-- being averaged into the response figures. A half-hidden job is worse than
-- a visible one — it is gone from the list you would check it against.
--
-- The `js` CTE already excludes drafts, abandoned rows and hidden ones; these
-- six now join through it. Body otherwise byte-identical to what was live.
-- ════════════════════════════════════════════════════════════════════════
