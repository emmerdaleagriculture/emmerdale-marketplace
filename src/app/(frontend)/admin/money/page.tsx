...
export default async function MoneyPage() {
  ...
  return (
    <div>
      ...
      <Tiles>
        <Tile value={formatGBP(totalHeld)} label="Collected on live jobs" hint={<>{held.length} payments on jobs not yet complete</>} />
        <Tile value={formatGBP(totalOwed)} label="Balances outstanding" hint={<>{owed.length} awaiting collection{stuck > 0 ? ` · ${stuck} failed` : ''}</>} />
        <Tile value={formatGBP(rows.filter((r) => r.status === 'paid').reduce((s2, r) => s2 + r.amount_pence, 0))} label="Taken (recent)" hint={<>last {rows.length} payments shown below</>} />
      </Tiles>
      ...
      {rows.length === 0 ? (
        ...
        <AdminTable head={['Job', 'Part', 'Amount', 'Job total', 'Contractor gets', 'Payment', 'Job status', 'When']}>
          {rows.map((r) => {
